...
def mainWindow():
    rootWindow = Tk()
    rootWindow.title("Problem pośrednika")
    ...
    def submitInputData():
        ...
        def calculate():
            ...
            suppliersSupplyArray = np.zeros([numberOfSuppliers])
            suppliersSellingPriceArray = np.zeros([numberOfSuppliers])

            for i in range(numberOfSuppliers):
                    suppliersSupplyArray[i] = suppliersArray[i][0]
                    suppliersSellingPriceArray[i] = suppliersArray[i][1]

            print(suppliersSupplyArray)
            print(suppliersSellingPriceArray)
            ...
            customersDemandArray = np.zeros([numberOfCustomers])
            customersPurchasingPriceArray = np.zeros([numberOfCustomers])

            for i in range(numberOfCustomers):
                    customersDemandArray[i] = customersArray[i][0]
                    customersPurchasingPriceArray[i] = customersArray[i][1]

            print(customersDemandArray)
            print(customersPurchasingPriceArray)
